  ...
  it("serves rolling stream metrics", async () => {
    ...
    try {
      ...
      expect(await response.json()).toEqual({
        ...
        games: [
          {
            broadcastLoopDurationMillisAvg: expect.any(Number),
            broadcastLoopDurationMillisMax: expect.any(Number),
            broadcastLoopDurationMillisP50: expect.any(Number),
            broadcastLoopDurationMillisP95: expect.any(Number),
            broadcastLoopDurationMillisP99: expect.any(Number),
            clients: 1,
            entityCountAvg: expect.any(Number),
            gameId: "game:1",
            running: true,
            requestedSimulationMillisPerSecond: expect.any(Number),
            simulationBacklogMillis: expect.any(Number),
            simulationMillisPerSecond: expect.any(Number),
            simulationStepsPerSecond: expect.any(Number),
            simulationThroughputRatio: expect.any(Number),
