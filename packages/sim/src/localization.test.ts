...
describe("Solitude localization", () => {
  it("omits thousands separators from Spanish numeric formatters", () => {
    ...
    expect(
      localization.lobby.gameSummary({
        assignedEntityIds: ["ship:blue"],
        availableEntityIds: [],
        gameId: "game:1",
        tick: 123_456,
      }),
    ).toBe("game:1 | tick 123456 | asignados ship:blue | disponibles ninguno");
  });

  it("localizes built-in entity names and preserves explicit display names", () => {
    const localization = createSolitudeLocalization("es");

    expect(localization.formatEntityName("planet:earth", undefined)).toBe(
      "Tierra",
    );
    expect(localization.formatEntityName("planet:custom", undefined)).toBe(
      "Custom",
    );
    expect(localization.formatEntityName("planet:earth", "Terra Prime")).toBe(
      "Terra Prime",
    );
  });
});
