...
- `packages/sim/src/`: browser-safe and Node-safe Solitude simulation library. Its remaining static plugin catalog owns `solarSystem`, `spacecraftOperator`, `autopilot`, and browser-only `autopilotInput`; it also provides the Solitude headless composition currently shared by authoritative multiplayer and tests.
...
- `packages/solitude/src/`: standalone browser app bootstrap and the remaining static product plugins: `pause`, `playback`, and `timeScale`.
...
- `plugins/standalone-pack-v1/`: independently built standalone-only browser plugin pack containing default ships, memory telemetry, profiling controls/HUD, and runtime operator focus switching.
...
- The headless autopilot plugin in `@solitude/sim` contributes control behavior and capabilities, while its sibling input plugin contributes browser keyboard bindings. The external `autopilotHud` plugin in `core-pack-v1` owns the localized HUD panel and message bundles; server/headless composition does not instantiate either browser-facing plugin.
...
- The external `standalone-pack-v1` package contributes `ships`, `memory`, `profiling`, and `operatorSwitch` only to standalone. It follows the shared packs; `operatorSwitch` remains after playback in final plugin order so it can refresh the scene when focus changes while playback is paused. Multiplayer's assembled plugin set and distribution do not contain the pack.
...
- `plugins/standalone-pack-v1/src/`: standalone-only external factories for ships, memory telemetry, profiling, and operator focus switching.
...
- `packages/solitude/src/plugins/pause/` and `packages/solitude/src/plugins/timeScale/`: remaining static standalone loop/input/HUD plugins.
