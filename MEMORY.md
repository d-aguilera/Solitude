...
- External plugins can declare `requirements.focusEntity` for focused-entity capabilities not guaranteed by `ExternalFocusContext`; the external runtime translates them to the engine's internal `mainFocus` requirement scope, and DOM/headless setup validates them against the assembled world with hard setup errors. External contribution callbacks are grouped under `ExternalPlugin.hooks`. API v9 retains the browser lifecycle and frozen snapshot facade, and permits authoritative `controls` plus pre-runtime `worldModel` hooks. Server requirements and simulation, loop, scene, view, and presentation hooks remain rejected.
