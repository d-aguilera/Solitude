...
- `plugins/core-pack-v1/`: independently built first-party browser plugin pack discovered at runtime by standalone and remote clients; currently contains orbit-segment, solar-system-material, targeting-laser, and velocity-segment plugins plus the material plugin's texture assets.
...
- The external `core-pack-v1` package currently contributes four client-side plugins to standalone and remote play. Orbit segments toggle with `G` and render the focused entity's bound analytic orbit around its dominant gravity body. Solar-system materials applies Earth and Moon texture materials and resolves three pack-owned JPEG assets relative to its loaded module. Targeting laser toggles with `T`, locks the collision sphere nearest the focused ship's nose axis, and renders a beam, target-plane miss guide, obstruction cue, or constant-screen-size surface impact marker. Velocity segments render forward/backward world-space lines along the focused entity's velocity. Both browser products discover the pack through same-origin `plugins/loader.json`; neither host statically depends on its package.
