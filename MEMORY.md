...
- **External plugin imports and trust**: packages under `plugins/*` may import only `@solitude/plugin-api` from the host workspace. Their emitted browser modules must be self-contained and are loaded through versioned manifests with no static host fallback. Browser loading starts from same-origin `plugins/loader.json`; every resolved document/module origin must be explicitly allowed, JSON redirects fail, and page CSP defaults plugin execution to `script-src 'self'`.
...
- `packages/solitude/src/`: Solitude standalone browser app bootstrap, static product plugin catalog, playback, runtime telemetry, HUD/readout behavior, and product-specific browser UX.
- `plugins/core-pack-v1/`: independently built first-party browser plugin pack discovered at runtime by standalone and remote clients; currently contains autopilot-HUD, axial-view, body-label, orbit-segment, orbit-telemetry, ship-telemetry, solar-system-material, targeting-laser, trajectory, and velocity-segment plugins plus the material plugin's texture assets.
