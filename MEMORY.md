...
- The external `solitude-content-pack-v1` is an atomic browser-and-server pack. It currently contributes `polyFighter`, which owns its OBJ mesh, derived mass, and controllable-entity provider. Both browser products discover the whole pack through their browser plugin sets; authoritative multiplayer discovers the same pack through `dist/server/plugins/plugin-set.json` and activates all of its plugins before creating sessions.
