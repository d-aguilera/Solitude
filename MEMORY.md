...
- `plugins/standalone-pack-v1/`: independently built standalone-only browser plugin pack, currently containing runtime operator focus switching.
...
- The external `solitude-content-pack-v1` is an atomic browser-and-server pack. It currently contributes `polyFighter`, which owns its OBJ mesh, derived mass, and controllable-entity provider. Both browser products discover the whole pack through their browser plugin sets; authoritative multiplayer discovers the same pack through `dist/server/plugins/plugin-set.json`, activates all of its plugins before creating sessions, and requires exactly one discovered controllable-entity provider without knowing its concrete id.
...
- The external `standalone-pack-v1` package contributes `operatorSwitch` only to standalone. It follows the shared packs so its loop hook remains after playback and can refresh the scene when focus changes while playback is paused. Multiplayer's assembled plugin set and distribution do not contain the pack.
...
- `plugins/standalone-pack-v1/src/`: standalone-only external runtime plugin factories, currently operator focus switching.
