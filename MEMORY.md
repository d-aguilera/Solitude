...
- Server remote client lives in `packages/solitude/remote.html`; `npm run dev:server` serves it through `@solitude/server/http` so it can create/join games, send server-authoritative controls, run/pause the authoritative world, receive WebSocket snapshots, interpolate them locally, and render through `@solitude/browser`.
...
- Active path: client-server architecture; server/session/protocol, WebSocket interactive transport, production-like asset serving, and browser remote rendering are in place. See `MEMORY_CLIENT_SERVER.md`.
