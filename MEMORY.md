...
- `plugins/core-pack-v1/`: independently built first-party plugin pack discovered at runtime by standalone and remote clients, with universal entries loadable by the authoritative server from deployed local manifests. It contains autopilot-HUD, axial-view, body-label, main-view-lookaround, orbit-segment, orbit-telemetry, poly-fighter, runtime-telemetry, ship-telemetry, solar-system-material, targeting-laser, trajectory, and velocity-segment plugins plus pack-owned model/texture assets.
...
- External plugin sets expand ordered pack manifests, and each independently built pack may contribute multiple ordered runtime plugin manifests. All packs and plugin manifests use strict schema/id validation; plugin API/environment and allowed-origin validation complete before module import. The loader config is a fixed same-origin trust root, defaults to `self`, and JSON fetches reject redirects. Missing, disallowed, incompatible, duplicate, or colliding plugins fail browser startup.
- Plugin deployment assembly is target-specific: `plugins/browser-plugin-packs.json` defines standalone and multiplayer pack order, `dist/plugin-public/<target>` stages each public tree, and each Vite product copies only its own tree.
...
- `packages/plugin-runtime/src/server.ts`: explicitly configured local server-plugin loading with manifest-environment checks and real-path containment for module entries.
- `packages/multiplayer/src/serverPlugins.ts`: authoritative startup composition that resolves the deployed core-pack root and loads the universal poly-fighter provider before creating sessions.
