...
function fromUp(referenceUp: Vec3): LocalFrame {
 ...
 */
function fromMat3(R: Mat3): LocalFrame {
  return alloc.withName(fromMat3.name, () => {
    const R0 = R[0];
    const R1 = R[1];
    const R2 = R[2];
 ...
 */
function toMat3(frame: LocalFrame): Mat3 {
  return alloc.withName(toMat3.name, () => {
    return intoMat3(mat3.zero(), frame);
  });
}

function intoMat3(into: Mat3, frame: LocalFrame): Mat3 {
...
function rotateAroundAxis(
  ...
  return alloc.withName(rotateAroundAxis.name, () => {
